...
- файл `/opt/krunker/apps/client/.env.production` с `VITE_SERVER_URL=https://твой-домен` либо `http://89.169.162.158` для тестового запуска без домена;
- сайт в `/var/www/krunker`;
- служба `krunker.service`, которая работает от пользователя `fps`;
- настроенный Caddy и рабочий сайт по выбранному адресу.
